...
void ft_zoom(t_fdf *fdf, int key)
{
    ...
    if ((fdf->zoom > -15 && key == 78) || (fdf->zoom < 15 && key == 69))
    {
        while (++j < fdf->y_max)
        {
            ...
            while (++i < fdf->x_max)
            {
                fdf->map[j][i].x = (fdf->map[j][i].x - (double)fdf->x0)  * (key == 69 ? 1.2 : (1 / 1.2));
                fdf->map[j][i].y = (fdf->map[j][i].y - (double)fdf->y0)  * (key == 69 ? 1.2 : (1 / 1.2));
...
void ft_colour(t_fdf *fdf, int key)
{
	...
	while (++j < fdf->y_max)
	{
		...
		while (++i < fdf->x_max)
		{
            fdf->map[j][i].x = (fdf->map[j][i].x - (double)fdf->x0);
            fdf->map[j][i].y = (fdf->map[j][i].y - (double)fdf->y0);
			fdf->map[j][i].colour = key == 15 ? 0xFF0000 : fdf->map[j][i].colour;
            fdf->map[j][i].colour = key == 5 ? 0x00FF00 : fdf->map[j][i].colour;
            fdf->map[j][i].colour = key == 11 ? 0x0000FF : fdf->map[j][i].colour;
            fdf->map[j][i].colour = key == 13 ? 0xFFFFFF : fdf->map[j][i].colour;
        }
	}
    mlx_clear_window(fdf->mlx_ptr, fdf->win_ptr);
    ft_set_print(fdf, -1, -1);
